    ...
    connect.query('SELECT * FROM `la_user` WHERE `username`= "' + req.body.username + '" AND `password` = "' + password + '"', function(err, row){
        if(row.length == 0){
            ...
            if(!req.session.TransaksiSession){ // if transaksisession is null program will access this block
                connect.query(`SELECT * FROM la_brgTransaksi WHERE username = "${req.session.userLogin.username}" ORDER BY id_barang`, (err, data) => {
                    if(data.length != 0){
                ...
                connect.query(`SELECT * FROM la_brgTransaksi WHERE username = '${req.session.userLogin.username}'`, (err, data) => { // check if in database was a data, if yes program will execute 
                    if(data.length != 0){ // where user was adding data and their have data in database
                        let isYourSessionSameInDb = Boolean;
                        let inDB = Number;
                        let inSession = Number;
                        insertToDB = "INSERT INTO la_brgTransaksi (username, id_barang, jumlah) VALUES "
                        ...
                        connect.query(`DELETE FROM la_brgTransaksi WHERE username = "${req.session.userLogin.username}"`, (err, data) => {
                            ...
                            if(data){
                                ...
                                connect.query(`SELECT * FROM la_brgTransaksi WHERE username = "${req.session.userLogin.username}" ORDER BY id_barang`, (err, data) => {
                                    if(data.length !== 0){
                                        ...
                                        console.log("Data di array Transaksi Cart : " + arrayTransaksiInCart);
                                        console.log(data[1].id_barang)
                                        req.session.TransaksiSession = arrayTransaksiInCart;
